...
const updateInList = (_id,data) => todoService.updateInList ({_id},data);
module.exports = {
    push : (req,res)=>{
    ...
    getAllToDoListByUser : (req,res)=>{
        let user_id = id(req.headers['user_id']);
        todoService.findinlist({user_id},{action:false})
        .then(data=>res.json({data,success:true}))
        .catch(err=>res.json({err,success:false}))
    },
    deleteToDo : (req,res)=>{
